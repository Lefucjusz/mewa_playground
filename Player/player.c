...
void player_init(I2S_HandleTypeDef *i2s, I2C_HandleTypeDef *i2c, void (*on_stop_callback)(void))
{
	ctx.buffer_req = BUFFER_REQ_NONE;
	ctx.state = PLAYER_STOPPED;
	ctx.i2s = i2s;
	ctx.i2c = i2c;
	ctx.on_stop_callback = on_stop_callback;
}
...
int player_start(const char *path)
{
	...
	if (!is_extension(path, ".mp3")) { // TODO add more formats
		return -ENOTSUP;
	}
...
void player_task(void)
{
	...
	if (frames_read == 0) {
		player_stop();
		if (ctx.on_stop_callback != NULL) {
			ctx.on_stop_callback();
		}
	}
}
